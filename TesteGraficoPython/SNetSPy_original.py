...
def extractDFS(path, arq, metric, alpha):
    ...
    print(solutions)
    labels = [s.split("_", 1)[0] for s in solutions]
    ...
    errorIntervals = [
        st.t.interval(1 - alpha, numRep - 1, loc=avg[i], scale=sd[i])
        for i in range(len(avg))
    ]
    dfs = [pd.DataFrame() for i in range(len(avg))]
    errors = [
        [errorIntervals[i][1].tolist()[j] - avg[i][j] for j in range(len(avg[i]))]
        for i in range(len(avg))
    ]  # error = limSup - mean
    errors = sd
    for i in range(len(avg)):
...
#     trr1 = [
#         d.groupby(["LoadPoint"]).sum()[list(d.filter(like="rep", axis=1))] for d in trr1
#     ]
#     trr2 = [d[d.Metrics == MRUPN] for d in trr]  # filter metric
#     trr2 = [
#         d.groupby(["LoadPoint"]).sum()[list(d.filter(like="rep", axis=1))] for d in trr2
#     ]
...
#     p = path
#     timeAjust = 31536000
#     arq1 = "_BitRateBlockingProbability.csv"
...
#     bcr = [
#         (((1 - bbp[i]) * grb[i]) * vbt) / (ce[i] * cws)
#         for i in list(range(0, len(solutions)))
#     ]
...
def plotLines(path, loads=[], sol=[], al=0.05):
    ...
    n = n + "_Line_"
    # PBB
    dfs, labels = extractDFS(path, ABBP, MBBP, al)
    if loads == []:
    ...
    auxPlotLine(dfs, loads, sol, xl, yl="Bit Rate Blocking", show=True, arq=a)
...
def plotBars(path, loads=[], sol=[], al=0.05):
    ...
    # a = path + "/" + n + "TXU.png"
    # auxPlotBars(
    #     dfs, loads, sol, xl, yl="Average Transponder Utilization", show=False, arq=a
    # )
    ...
    # a = path + "/" + n + "MBVTU.png"
    # auxPlotBars(
    #     dfs, loads, sol, xl, yl="Maximum Number of BVTs Used", show=False, arq=a
    # )
    ...
    # a = path + "/" + n + "MRC.png"
    # auxPlotBars(
    #     dfs, loads, sol, xl, yl="Maximum requests by circuit", show=False, arq=a
    # )
...
#     gains = [
#         minGains(extractDFS(path, arqs[i], metr[i], 0.05), ims)
...
#     metr = [MRRC]
#     gains = [
#         minGains(extractDFS(path, arqs[i], metr[i], 0.05), ims, inv=True)
#         for i in range(len(arqs))
#     ]
#     for i in range(len(metr)):
#         df[metr[i]] = gains[i]
...
def auxPlotLine(
    dfs, loads, sol, xl="", yl="", show=True, arq="", lp="lower center", nc=5
):
    print(dfs)
    fs = 27
...
# plotLine("/Users/gustavo/Desktop/Teste Grafico Python/simulations","sol","sol")
